...
- `layer_0`: 通常使用するQWERTYの基本入力レイヤー。`r1 c0` は左Ctrl。`r2 c6` の短押しでMacの左のデスクトップ（Spaces）へ移動（Control＋←）、`r2 c7` の短押しで右へ移動（Control＋→）
- `layer_1`: クリックや戻る・進むを操作する一時マウスレイヤー
- `layer_2`: F1-F12、JISの1（Shiftで!）、Deleteを配置したトラックボールのスクロールレイヤー
- `layer_3`: ファンクションキー、ナビゲーション、出力切替をまとめたレイヤー。左端列の `r0 c0`〜`r3 c0` は上からBLEプロファイル0〜3を選択（`&bt BT_SEL 0`〜`3`）
